        ...
        <div className="relative overflow-hidden rounded-md border border-border/70 bg-card">
          <div className="grid min-h-[28rem] grid-cols-1 lg:grid-cols-[1.25fr_0.75fr]">
            <div className="relative min-h-[20rem] overflow-hidden lg:min-h-[30rem]">
              {slides.map((slide, index) => <img key={slide.id} src={slide.image?.startsWith("/") ? `/d${slide.image}` : slide.image || "/d/placeholder.svg"} alt={`${slide.name} in ${slide.location}`} className={`absolute inset-0 size-full object-cover transition-opacity duration-700 ease-luxe ${index === safeActive ? "opacity-100" : "opacity-0"}`} />)}
